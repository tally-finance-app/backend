// Module tools exists only to pin the versions of development tools (sqlc,
// golangci-lint, lefthook) WITHOUT letting their dependency graphs influence the
// main module's runtime dependencies.
//
// Go excludes any directory containing its own go.mod from the parent module, so
// nothing here is ever compiled into the application. Tools are built into ../bin
// by the Makefile — see the `bin/%` rule there.
//
// Do NOT add this module to a go.work workspace: workspaces unify minimal version
// selection across modules, which would re-merge these dependencies into the main
// module's build list and defeat the entire point of the split.
module github.com/tally-finance-app/backend/tools

go 1.26.5

require github.com/sqlc-dev/sqlc v1.31.1 // indirect
